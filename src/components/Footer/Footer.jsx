import PropTypes from "prop-types";

import TasksFilter from "../TasksFilter";

import "./Footer.css";

const Footer = ({ setTodoData, tasksCount, filterFunc, filterFlag }) => {
  const clearTaskCompleted = () => {
    setTodoData((data) => data.filter((el) => !el.completed));
  };

  return (
    <footer className="footer">
      <span className="todo-count">{tasksCount}items left</span>

      <TasksFilter filterFunc={filterFunc} filterFlag={filterFlag} />

      <button onClick={clearTaskCompleted} className="clear-completed">
        Clear completed
      </button>
    </footer>
  );
};

Footer.prototype = {
  tasksCount: PropTypes.number.isRequired,
  filterFlag: PropTypes.string.isRequired,
  setTodoData: PropTypes.func.isRequired,
  filterFunc: PropTypes.func.isRequired,
};

export default Footer;
